...
def semi_loss(tao, Z1: torch.Tensor, Z2: torch.Tensor):
    f = lambda x: torch.exp(x / tao)
...
def instanceloss(tao, Z1: torch.Tensor, Z2: torch.Tensor, mean: bool = True):
    l1 = semi_loss(tao, Z1, Z2)
    l2 = semi_loss(tao, Z2, Z1)
    ret = (l1 + l2) * 0.5
    ret = ret.mean() if mean else ret.sum()
    return ret


def knbrsloss(H, k, tao, node_num, device):
    nbrs = NearestNeighbors(n_neighbors=k + 1, algorithm="auto").fit(H.cpu().detach().numpy())
    _, indices = nbrs.kneighbors(H.cpu().detach().numpy())

    refl_sim = torch.exp(sim(H, H) / tao)
    V = torch.zeros((node_num, k)).to(device)

    for i in range(node_num):
        for j in range(k):
            V[i][j] += refl_sim[i][indices[i][j + 1]]  # 修正赋值方式

    pos_similarity_sum = V.sum(1)
    neg_similarity_sum = refl_sim.sum(1) - refl_sim.diag()

    # 计算损失
    ret = -torch.log(pos_similarity_sum / neg_similarity_sum)
    ret = ret.mean()
...
def Modula(array, cluster):
    m = array.sum() / 2
    k1 = torch.sum(array, axis=1)
    k2 = k1.reshape(k1.shape[0], 1)
    k1k2 = k1 * k2
    Eij = k1k2 / (2 * m)
    B = array - Eij
    results = cluster.T @ B @ cluster
    sum_results = torch.trace(results)
